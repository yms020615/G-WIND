...
class ERA5Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        **kwarg,  # pylint: disable=unused-argument
        ...
        if "example" in dataset_name:
            ...
            split_slices = {
                "train": slice("2006-01-01T12", "2015-12-31T12"),  # 1959-2017
                "val": slice("2015-12-31T18", "2016-12-31T12"),  # 2018-2019
            }
            if expanded_test:
                # 2020-2023
                ...
                split_slices["test"] = slice("2020-09-30T18", "2020-11-10T18")
    ...
    def __getitem__(self, idx):
        ...
        sample_times = [
            (np.datetime64(t) - np.datetime64('2006-01-01T12')) / np.timedelta64(1, 's')
            for t in self.time_coords[sample_slice]
        ]
        sample_times_torch = torch.tensor(sample_times, dtype=torch.float32)

        return init_states, target_states, forcing_torch, sample_times_torch
